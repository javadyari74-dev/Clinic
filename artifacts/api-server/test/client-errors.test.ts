...
describe("POST /client-errors de-duplication", () => {
  it("records an identical message+url only once within the window, but records distinct reports", async () => {
    ...
    // Dropped duplicates are acknowledged with 204 until the same error+page
    // has been suppressed PERSISTENT_CRASH_THRESHOLD times — then the server
    // answers 200 + { persistentCrash } so the client raises a visible
    // operator warning (the error-boundary depends on this contract).

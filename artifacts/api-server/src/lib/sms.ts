...
export const SMS_SETTING_KEYS = {
  ...
  // خوش‌آمد به معرف تازه‌ثبت‌شده (کمیسیون‌گیرنده)
  enabledRecipientWelcome: "sms_enabled_recipient_welcome",
...
export async function getSmsSettings(): Promise<SmsSettings> {
  ...
  return {
    ...
    enabledRecipientWelcome: flag(SMS_SETTING_KEYS.enabledRecipientWelcome),
...
// خوش‌آمد به معرف (کمیسیون‌گیرنده) تازه‌ثبت‌شده — پس از ثبت نام او صدا زده می‌شود.
export function fireRecipientWelcomeSms(args: {
  name: string | null;
  phone: string | null;
}): void {
  void (async () => {
    try {
      const settings = await getSmsSettings();
      if (!settings.enabledRecipientWelcome) return;
      if (!args.phone) return;
      const templates = await getSmsTemplates();
      const name = args.name ?? "";
      const text = renderTemplate(templates.recipientWelcome, { "نام": name });
      await sendSms({
        to: args.phone,
        text,
        eventType: "recipient_welcome",
        recipientName: args.name,
        // ترتیب متغیرها: PATTERN_VAR_ORDER.recipientWelcome
        pattern:
          settings.sendMode === "pattern"
            ? { bodyId: settings.bodyIdRecipientWelcome, args: [name] }
            : undefined,
      });
    } catch (err) {
      logger.warn({ err }, "fireRecipientWelcomeSms failed");
    }
  })();
}

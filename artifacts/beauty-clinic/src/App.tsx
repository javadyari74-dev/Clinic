...
const Sms = lazy(pageLoaders.sms);
...
// A stored token can become invalid (expired, or the server's signing secret
// changed). Every request then fails with 401 and, without this handler, the
// UI would just show generic "load failed" errors. Instead: drop the stale
// token and reload so the user lands on the login page.
function handleUnauthorized(error: unknown) {
  const status = (error as { status?: number } | null)?.status;
  if (status !== 401) return;
  if (!localStorage.getItem("clinic_auth_token")) return;

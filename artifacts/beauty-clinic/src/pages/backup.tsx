...
import {
  Download, Database, Shield, Trash2, AlertTriangle, ShieldAlert, Upload, RotateCcw,
  GitMerge, FolderCog, Save, History, CheckCircle2, XCircle,
} from "lucide-react";
...
type MergeReportEntry = { added: number; skipped: number };
type MergeReport = Record<string, MergeReportEntry>;
...
export default function Backup() {
  ...
  async function loadLogs() {
    try {
      const res = await fetch(`${BASE_URL}/api/backup/logs`, {
        headers: { "Authorization": `Bearer ${token}` },
      });
      if (!res.ok) return;
      const data = await res.json();
  ...
  async function saveBackupDir() {
    ...
    try {
      ...
      setIsDefaultDir(!!data.isDefault);
      setBackupDir(data.isDefault ? "" : (data.backupDir ?? ""));
      toast({ title: data.message || "مسیر ذخیره بکاپ ذخیره شد" });
    } catch (err) {
  ...
  function formatLogDate(sec: number) {
    try {
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Button className="w-full gap-2" onClick={saveBackupDir} disabled={savingDir}>
              ...
              {logs.map(log => (
                <div key={log.id} className="flex items-center gap-3 p-2.5 text-sm">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="font-medium truncate" dir="ltr">{log.filename ?? "—"}</div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y">
                    {Object.entries(mergeReport)
                      ...
                      .map(([key, entry]) => (
                        <tr key={key}>
                          <td className="p-2">{SECTION_LABELS[key] ?? key}</td>
                          <td className="p-2 text-center font-medium text-emerald-600">{entry.added}</td>
                          <td className="p-2 text-center text-muted-foreground">{entry.skipped}</td>
                        </tr>
                      ))}
                  </tbody>
                  {mergeReport.totals && (

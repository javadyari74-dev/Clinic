...
import {
  ...
  useConvertWaitingEntry, useNotifyWaitingEntry, getListWaitingListQueryKey,
} from "@workspace/api-client-react";
import type { WaitingEntry } from "@workspace/api-client-react";
...
import { Check, ChevronsUpDown, Plus, Pencil, Trash2, CalendarCheck, BellRing } from "lucide-react";
...
const waitingStatuses: Record<string, { label: string; color: string }> = {
...
type WaitingFormValues = z.infer<typeof waitingSchema>;

function dateStrToSec(date: string): number {
  const [year, month, day] = date.split("-").map(Number);
  return Math.floor(new Date(year, month - 1, day, 12, 0, 0).getTime() / 1000);
}

function secToDateStr(ts: number): string {
  const d = new Date(ts > 1e11 ? ts : ts * 1000);
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}

function waitingRange(entry: WaitingEntry): string {
...
function WaitingListSection({ onConvert }: { onConvert: (entry: WaitingEntry, date?: string) => void }) {
  ...
  const { data: list, isError, refetch } = useListWaitingList(statusFilter === "all" ? undefined : { status: statusFilter });
  ...
  const form = useForm<WaitingFormValues>({
  ...
  function onSubmit(values: WaitingFormValues) {
    const base = {
    ...
    if (editEntry) {
      updateEntry.mutate({
        id: editEntry.id,
        data: { ...base, preferredFrom: base.preferredFrom ?? null, preferredTo: base.preferredTo ?? null, note: base.note ?? null },
      });
    } else {
      createEntry.mutate({ data: base });
    }
  }
    ...
    <Card>
      <CardHeader className="pb-3 flex-row items-center justify-between space-y-0">
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-[220px]"><SelectValue placeholder="در انتظار" /></SelectTrigger>
          <SelectContent>
              ...
              <TableRow key={entry.id}>
                <TableCell className="font-medium">
                  ...
                  <span className="block text-xs text-muted-foreground">
                    {entry.patientPhone ? toPersianDigits(entry.patientPhone) : ""}
                  </span>
                </TableCell>
                <TableCell>{entry.serviceName}</TableCell>
                <TableCell className="text-sm">{waitingRange(entry)}</TableCell>
                ...
                <TableCell>
                  <div className="flex gap-1 justify-end">
                    {entry.status === "waiting" && (
                      <>
                        <Button size="sm" variant="outline" className="h-7 gap-1 text-xs" title="ثبت نوبت برای این مراجع" onClick={() => onConvert(entry)}>
                          <CalendarCheck className="h-3.5 w-3.5" />
                          ثبت نوبت
                        </Button>
                        <Button size="sm" variant="outline" className="h-7 gap-1 text-xs" title="اطلاع‌رسانی جای خالی با پیامک" disabled={notifyingId === entry.id} onClick={() => handleNotify(entry)}>
                    ...
                    <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-destructive" onClick={() => setDeleteId(entry.id)}>
                      <Trash2 className="h-3.5 w-3.5" />
                    </Button>
                  </div>
                </TableCell>
      ...
      {/* Create / edit waiting entry dialog */}
      <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) setEditEntry(null); }}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{editEntry ? "ویرایش مورد لیست انتظار" : "افزودن به لیست انتظار"}</DialogTitle></DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormField control={form.control} name="note" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Textarea placeholder="مثال: فقط بعدازظهرها" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <DialogFooter>
                <Button type="submit" disabled={createEntry.isPending || updateEntry.isPending} className="w-full">
                  {createEntry.isPending || updateEntry.isPending ? "در حال ذخیره..." : editEntry ? "ذخیره تغییرات" : "افزودن به لیست انتظار"}
                </Button>
              </DialogFooter>
            </form>
          </Form>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => { if (deleteId !== null) deleteEntry.mutate({ id: deleteId }); }}
            >
...
export default function Appointments() {
  ...
  const pendingConvert = useRef<WaitingEntry | null>(null);
  ...
  const convertWaiting = useConvertWaitingEntry({
    mutation: {
      onSuccess: () => {
        invalidate();
        invalidateWaiting();
  ...
  function onSubmit(values: FormValues) {
    const entry = pendingConvert.current;
    if (entry && values.patientId === entry.patientId) {
      convertWaiting.mutate({
        id: entry.id,
        data: {
  ...
  function handleConvertWaiting(entry: WaitingEntry, date?: string) {
    pendingConvert.current = entry;
    form.reset({
      patientId: entry.patientId,
      serviceId: entry.serviceId,
      date: date ?? (entry.preferredFrom ? secToDateStr(entry.preferredFrom) : todayString()),
      time: "09:00",
      hasDeposit: false,
      depositAmount: 0,
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={isOpen} onOpenChange={(open) => { setIsOpen(open); if (!open) pendingConvert.current = null; }}>
        <DialogContent className="max-w-md">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <DialogFooter>
                <Button type="submit" disabled={createAppointment.isPending || convertWaiting.isPending} className="w-full">
                  {createAppointment.isPending || convertWaiting.isPending ? "در حال ثبت..." : "ثبت نوبت"}
                </Button>
              </DialogFooter>
            </form>
          </Form>
      ...
      <Tabs defaultValue="active">
        ...
        <TabsContent value="waiting">
          <WaitingListSection onConvert={handleConvertWaiting} />
        </TabsContent>
      </Tabs>
    </div>
  );

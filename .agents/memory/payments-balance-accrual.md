...
**Deleting/refunding a payment must reverse the same financial effects, atomically.** `DELETE /payments/:id` runs in one `db.transaction`: it finds every `patient_account_transactions` row with `paymentId == id` and restores each owner's balance via `accountBalance -= txn.amount` (amount is signed, so a `deduct` of -X adds X back and a `referral_credit` of +Y removes Y), deletes those txns, deletes any `commissions` row with `paymentId == id`, then deletes the payment. Reversal is keyed on `paymentId`, so both the deduct (payer) and the referral_credit (referrer) get reversed together even though they sit on different patients.
- For this to work the links MUST be set at creation: the server already stamps `paymentId` on the `deduct` txn and on the auto staff/external `commissions` row; the patient-referrer `referral_credit` txn is stamped by the frontend (`createAccountTxn` passes `paymentId: payment.id`). `commissions.payment_id` was added (migration 0008) precisely so the auto-commission can be reversed without touching manually-created commissions on the same appointment.
- The manual/operator commission (frontend `createCommission`) deliberately carries no `paymentId`, so deleting a payment does NOT touch it — only the auto referrer accrual is reversed.

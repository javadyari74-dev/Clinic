...
- Deliberate exception: manual, user-initiated send endpoints (e.g. waiting-list vacancy notify) MAY await `sendSms` because the whole point is to report the send result to the receptionist; `sendSms` never throws and logs its own sms_log row, so the request can't fail because of it. Don't "fix" these to fire-and-forget.

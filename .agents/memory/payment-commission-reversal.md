...
## Loyalty points are another paymentId-linked side-effect
The باشگاه مشتریان feature writes `loyalty_transactions` rows (earn/redeem) carrying `payment_id` at checkout, and `DELETE /payments/:id` reverses them inside the same transaction (insert opposing rows), blocking deletion if the patient balance would go negative. Any new checkout side-effect must follow this same pattern: carry `paymentId`, reverse atomically on delete. Frontend: payment deletion must invalidate loyalty caches (overview + any query key ending in `/loyalty`) or the UI shows stale balances.

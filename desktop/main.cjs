...
// The server requires a JWT signing secret in production. Generate one once
// and persist it in userData so existing logins stay valid across restarts
// and app updates (changing the secret invalidates all stored tokens).
function getSessionSecret() {
  const secretPath = path.join(getUserDataDir(), "session-secret.txt");
  try {
  ...
  try {
    ...
    /* fall through — still return the in-memory secret for this run */
  }
  return secret;
}
